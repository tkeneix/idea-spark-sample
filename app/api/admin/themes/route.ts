...
export async function POST(request: NextRequest) {
  try {
    ...
    const theme = await Database.createTheme({ name, description, image_url })
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const theme = await Database.updateTheme(id, { name, description, image_url })
    return NextResponse.json({ theme })
  } catch (error) {
